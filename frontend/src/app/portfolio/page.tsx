...
export default function PortfolioPage() {
  ...
  return (
    <div className="min-h-screen pt-xxl bg-background">
      {/* Hero Section */}
      <section className="px-gutter py-xxl text-center max-w-container-max mx-auto">
        <div className="inline-flex items-center gap-xs px-md py-xs bg-surface-container-low rounded-full mb-md border border-outline-variant/30">
          <span className="material-symbols-outlined text-[18px] text-primary">auto_awesome</span>
          <span className="font-label-sm text-on-surface-variant uppercase tracking-widest">Case Studies</span>
        </div>
        <h1 className="font-headline-lg text-headline-lg md:font-display-lg md:text-display-lg text-on-surface mb-md">Our Portfolio</h1>
      ...
      <section className="px-gutter pb-xxl max-w-container-max mx-auto">
        <div className="flex flex-col md:flex-row items-center justify-between gap-md mb-xl">
          {/* Filter Bar */}
          <div className="flex flex-wrap items-center gap-xs">
            {categories.map((cat) => (
              <button
                ...
                className={`px-lg py-sm rounded-full border font-label-md transition-all cursor-pointer ${
                  filterCategory === cat
              ...
              <div key={proj.id} className="group relative overflow-hidden rounded-xl bg-white border border-surface-variant shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col h-full">
                ...
                <div className="p-lg flex flex-col flex-grow">
                  <div className="flex items-center gap-sm mb-sm">
                    <span className="px-sm py-xs bg-surface-container-low text-primary font-label-sm rounded-md uppercase tracking-wider">
                      {proj.category}
                    </span>
                  </div>
                  <h3 className="font-headline-md text-headline-md text-on-surface mb-sm">{proj.title}</h3>
                  ...
                  <div className="pt-sm border-t border-outline-variant/20 flex justify-between items-center mt-auto">
                    {proj.preview_url ? (
                      <a
